...
export async function getSpots(eventId: string): Promise<{event: EventModel, spots: SpotModel[]}> {
    const response = await axios.get(`http://localhost:8080/events/${eventId}/spots`)

    return response.data
}


export default async function EventSpots({ params: {eventId} }: {params: {eventId: string}}) {
    console.log(eventId)
    const { event, spots } = await getSpots(eventId)

    const rowLetter = spots.map(spot => spot.name[0])

    const uniqueRows = rowLetter.filter((row, index) => rowLetter.indexOf(row) === index)
